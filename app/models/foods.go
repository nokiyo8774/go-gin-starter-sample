...
var FoodWhere = struct {
	...
	ID:    whereHelperint64{field: "`foods`.`id`"},
	Name:  whereHelperstring{field: "`foods`.`name`"},
	Price: whereHelperuint{field: "`foods`.`price`"},
	Note:  whereHelpernull_String{field: "`foods`.`note`"},
}
...
var (
	foodAllColumns            = []string{"id", "name", "price", "note"}
	foodColumnsWithoutDefault = []string{"name", "price", "note"}
...
func FindFood(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Food, error) {
	foodObj := &Food{}
...
func FoodExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
